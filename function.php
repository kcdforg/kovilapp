...
function add_user($udata) {
    ...
    if (count($udata) && $udata['username'] != '') {
        ...
        //$creation_date = $udata['creation_date'];
        //$creation_date = "now()";
        $created_by = $udata['created_by'];

        $sql = "INSERT INTO `$tbl_users`(`username`, `email`, `password`, `role`, `profile_id`, `creation_date`, `created_by` ) VALUES ('$username', '$email', '$password', '$role', '$profile_id', now(), '$created_by')";
//echo $sql;die;
        return mysqli_query($con, $sql);
    }
}
