...
function buildFamilyTreeNode($family_id, &$visited = []) {
    ...
    // Fetch family details including name and DOB
    $sql_family = "SELECT id, name, w_name, dob, w_dob FROM $tbl_family WHERE id = ?";
    $stmt_family = mysqli_prepare($con, $sql_family);
    mysqli_stmt_bind_param($stmt_family, "i", $family_id);
    mysqli_stmt_execute($stmt_family);
    $result_family = mysqli_stmt_get_result($stmt_family);
    $family_data = mysqli_fetch_assoc($result_family);
    mysqli_stmt_close($stmt_family);
    ...
    $node = [
        ...
        'w_name' => $family_data['w_name'] ?? null,
        'dob' => $family_data['dob'] ?? null,
        'w_dob' => $family_data['w_dob'] ?? null,
        'children' => []
    ];
    
    // Get all children of this family with their details
    $sql = "SELECT id, fam_id, c_name, c_dob FROM $tbl_child WHERE father_id = ? ORDER BY c_dob ASC";
    ...
    while ($child = mysqli_fetch_assoc($result)) {
        // Create a child node with all necessary data
        $child_node = [
            'child_id' => $child['id'],
            'fam_id' => $child['fam_id'],
            'type' => 'child',
            'c_name' => $child['c_name'],
            'c_dob' => $child['c_dob'] ?? null,
            'children' => []
        ];
