...
global $con;
$result = mysqli_query($con, "SELECT DISTINCT w_kootam FROM $tbl_family WHERE w_kootam!=' 'ORDER BY w_kootam ASC");
?>
<style>
...
<div class="container-fluid">
    <h2 class="text-center">List By Kootam</h2>
</div>
<div id="list" class="col-md-12">
    <div id="list" class="col-md-2">
        <div class="row1">
            <h3 class="box-title"><b><center> Kootam </center></b></h3>
        </div>
        <?php
        if (!$result) {
            ...
            while ($row = mysqli_fetch_array($result)) {
                ...
                <li  id="listby"class="list-group-item">			
                    <b><a href="kwise.php?w_kootam=<?php echo $row['w_kootam'] ?>"><?php echo get_kulam($row['w_kootam']) ?></b></a>
                </li>
    ...
    if (isset($_GET['w_kootam'])) {
        $w_kootam = $_GET['w_kootam'];
        $con = connectdb();
        mysqli_set_charset($con, 'utf8');
        mysqli_select_db($con, "unicode");
        $count_res = mysqli_query($con, "SELECT count(*) as cnt FROM $tbl_family WHERE w_kootam='$w_kootam'");
        $count = mysqli_fetch_array($count_res);
        $total_records = $count['cnt'];
        $total_pages = ceil($total_records / $rec_per_page);
        $result = mysqli_query($con, "SELECT * FROM $tbl_family WHERE w_kootam='$w_kootam' LIMIT $offset, $rec_per_page");
    } else {
        $con = connectdb();
        mysqli_set_charset($con, 'utf8');
        mysqli_select_db($con, "unicode");
        $result = mysqli_query($con, "SELECT * FROM $tbl_family order by w_kootam");
            ...
            <div class="row">
                ...
                <div class="box table-responsive">
                    <div class="box-header">
                        <h3 class="box-title"><b>Search Results for  <?php echo $w_kootam; ?></b></h3>
                    </div>
                        ...
                        <div class="dataTables_paginate paging_simple_numbers pagination-flex-align" style="margin:10px 0 10px 0; justify-content: flex-end; display: flex; align-items: center; gap: 16px;">
                            <ul class="pagination" style="margin-bottom: 0; margin-right: 0; vertical-align: middle;">
                                ...
                                <li class="paginate_button previous<?php if ($curr_page <= 1 || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page > 1 && !$disable_all) ? 'kwise.php?w_kootam=' . urlencode($w_kootam) . '&page=' . ($curr_page-1) . '&per_page=' . $rec_per_page : '#'; ?>">Prev</a>
                                </li>
                                <?php for ($i = 1; $i <= max(1, $total_pages); $i++) { ?>
                                    <li class="paginate_button <?php if ($i == $curr_page) echo 'active'; if ($disable_all) echo ' disabled'; ?>">
                                        <a href="<?php echo (!$disable_all && $i != $curr_page) ? 'kwise.php?w_kootam=' . urlencode($w_kootam) . '&page=' . $i . '&per_page=' . $rec_per_page : '#'; ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php } ?>
                                <li class="paginate_button next<?php if ($curr_page >= $total_pages || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page < $total_pages && !$disable_all) ? 'kwise.php?w_kootam=' . urlencode($w_kootam) . '&page=' . ($curr_page+1) . '&per_page=' . $rec_per_page : '#'; ?>">Next</a>
                                </li>
                            </ul>
                            <form method="get" class="form-inline" style="margin-bottom:0; margin-left:0; vertical-align: middle;">
                                <input type="hidden" name="w_kootam" value="<?php echo htmlspecialchars($w_kootam); ?>">
