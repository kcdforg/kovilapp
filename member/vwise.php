...
global $con;
$result = mysqli_query($con, "SELECT DISTINCT village, count(*) as cnt  FROM $tbl_family  WHERE village  <> ' ' group by village ORDER BY village ASC");
$village='';
...
<div id="list" class="col-md-12">
    <div id="list" class="col-md-2" style="border-top: 0px;">
        <div class="row1">
            <h3 class="box-title"><b><center> Village </center></b></h3>
        </div>
        <?php
        if (!$result) {
            ...
            <li id="listby" class="list-group-item">			
             ...
             <?php
                ...
                <li  id="listby"class="list-group-item <?php echo $active ?>">		
                    <b><a href="vwise.php?village=<?php echo $row['village'] ?>"><?php echo $row['village'] ?> (<?php echo $row['cnt'] ?>)</b></a>
                </li>
    ...
    <?php
       $counter = 0;
          if($village){
     //get count
            $count_res = mysqli_query($con, "SELECT count(*) as cnt FROM $tbl_family WHERE  village='$village'");
            $count = mysqli_fetch_array($count_res);
            $total_records = $count['cnt'];
            $total_pages = ceil($total_records / $rec_per_page);
        $result = mysqli_query($con, "SELECT * FROM $tbl_family WHERE village='$village' LIMIT $offset, $rec_per_page");
        ?>
        <div id="list" class="col-md-10">
            <div  class="row">        
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><b>Search Results for  <?php echo $village; ?> (<?php echo $count['cnt']; ?>)</b></h3>
                    </div>
                        ...
                        <div class="dataTables_paginate paging_simple_numbers pagination-flex-align" style="margin:10px 0 10px 0; justify-content: flex-end; display: flex; align-items: center; gap: 16px;">
                            <ul class="pagination" style="margin-bottom: 0; margin-right: 0; vertical-align: middle;">
                                ...
                                <li class="paginate_button previous<?php if ($curr_page <= 1 || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page > 1 && !$disable_all) ? 'vwise.php?village=' . urlencode($village) . '&page=' . ($curr_page-1) . '&per_page=' . $rec_per_page : '#'; ?>">Prev</a>
                                </li>
                                <?php for ($i = 1; $i <= max(1, $total_pages); $i++) { ?>
                                    <li class="paginate_button <?php if ($i == $curr_page) echo 'active'; if ($disable_all) echo ' disabled'; ?>">
                                        <a href="<?php echo (!$disable_all && $i != $curr_page) ? 'vwise.php?village=' . urlencode($village) . '&page=' . $i . '&per_page=' . $rec_per_page : '#'; ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php } ?>
                                <li class="paginate_button next<?php if ($curr_page >= $total_pages || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page < $total_pages && !$disable_all) ? 'vwise.php?village=' . urlencode($village) . '&page=' . ($curr_page+1) . '&per_page=' . $rec_per_page : '#'; ?>">Next</a>
                                </li>
                            </ul>
                            <form method="get" class="form-inline" style="margin-bottom:0; margin-left:0; vertical-align: middle;">
                                <input type="hidden" name="village" value="<?php echo htmlspecialchars($village); ?>">
                    ...
                    $result = mysqli_query($con, "SELECT * FROM $tbl_family order by village");
