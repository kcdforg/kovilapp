...
if (isset($_GET['id']) && isset($_FILES['w_image'])) {
    ...
    if ($member_id > 0) {
        ...
        if ($_FILES['w_image']['error'] > 0) {
            $response['message'] = 'Upload error: ' . $_FILES['w_image']['error'];
            echo json_encode($response);
            exit;
        }
        ...
        // Generate filename
        $file_extension = pathinfo($_FILES['w_image']['name'], PATHINFO_EXTENSION);
        $filename = $member_id . "_wife." . $file_extension;
        $target_path = $upload_dir . $filename;
        
        // Delete old image if exists
        $old_images = glob($upload_dir . $member_id . "_wife.*");
        ...
        // Move uploaded file
        if (move_uploaded_file($_FILES['w_image']['tmp_name'], $target_path)) {
