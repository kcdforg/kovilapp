...
include('../header.php');
global $con;
$result_occ = mysqli_query($con, "SELECT DISTINCT occupation, count(*) as cnt FROM $tbl_family  WHERE occupation <> ' ' group by occupation ORDER BY occupation ASC");
    ...
    if (isset($_GET['occupation'])) {
        ...
        //get count
            $count_res = mysqli_query($con, "SELECT count(*) as cnt FROM $tbl_family WHERE  occupation='$occupation'");  
            $count = mysqli_fetch_array($count_res);
        
        $total_records = $count['cnt'];
        $total_pages = ceil($total_records / $rec_per_page);
        $result = mysqli_query($con, "SELECT * FROM $tbl_family WHERE  occupation='$occupation' LIMIT $offset, $rec_per_page");
    }
        ?>
<style>
...
<div id="list" class="col-md-12">
    <div id="list" class="col-md-2" style="border-top: 0px;">
        <div class="row1" >
            <h3 class="box-title"><b><center> Occupation </center></b></h3>
        </div>
        <?php
        if (!$result_occ) {
            ...
            while ($row = mysqli_fetch_array($result_occ)) {
                ...
                <li  id="listby"class="list-group-item <?php echo $active ?>">			
                    <b><a href="owise.php?occupation=<?php echo $row['occupation'] ?>"><?php echo get_occupation($row['occupation']) ?> (<?php echo $row['cnt'] ?>)</a></b>
                </li>
            ...
            <div class="row">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"><b>Search Results for  <?php echo get_occupation($occupation); ?> (<?php echo $count['cnt']; ?>)</b></h3>
                    </div>
                        ...
                        <div class="dataTables_paginate paging_simple_numbers pagination-flex-align" style="margin:10px 0 10px 0; justify-content: flex-end; display: flex; align-items: center; gap: 16px;">
                            <ul class="pagination" style="margin-bottom: 0; margin-right: 0; vertical-align: middle;">
                                ...
                                <li class="paginate_button previous<?php if ($curr_page <= 1 || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page > 1 && !$disable_all) ? 'owise.php?occupation=' . urlencode($occupation) . '&page=' . ($curr_page-1) . '&per_page=' . $rec_per_page : '#'; ?>">Prev</a>
                                </li>
                                <?php for ($i = 1; $i <= max(1, $total_pages); $i++) { ?>
                                    <li class="paginate_button <?php if ($i == $curr_page) echo 'active'; if ($disable_all) echo ' disabled'; ?>">
                                        <a href="<?php echo (!$disable_all && $i != $curr_page) ? 'owise.php?occupation=' . urlencode($occupation) . '&page=' . $i . '&per_page=' . $rec_per_page : '#'; ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php } ?>
                                <li class="paginate_button next<?php if ($curr_page >= $total_pages || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page < $total_pages && !$disable_all) ? 'owise.php?occupation=' . urlencode($occupation) . '&page=' . ($curr_page+1) . '&per_page=' . $rec_per_page : '#'; ?>">Next</a>
                                </li>
                            </ul>
                            ...
                            <tbody><?php
                                while ($row = mysqli_fetch_array($result)) {
                                    ...
                                    <tr>	
                                        ...
                                        <td  class="data"><?php echo str_replace(",", ", <br>", $row['permanent_address']) ?></td>
                                        <td ><?php echo $row['occupation_details'] ?></td>
                                        <td  ><?php echo $row['education_details'] . "<br>" . get_blood_group($row['blood_group']) . "<br>" . $row['pudavai'] ?></td>
                                    </tr>
                                    <?php
                                }
                                ?>
                ...
                $result = mysqli_query($con, "SELECT * FROM $tbl_family order by occupation");
                            ...
                            <div id="bb" class="box-body no-padding">
                                <div class="dataTables_paginate paging_simple_numbers pagination-flex-align" style="margin:10px 0 10px 0; justify-content: flex-end; display: flex; align-items: center; gap: 16px;">
                                    <ul class="pagination" style="margin-bottom: 0; margin-right: 0; vertical-align: middle;">
                                        <?php $disable_all = ($total_pages <= 1); ?>
                                        <li class="paginate_button previous<?php if ($curr_page <= 1 || $disable_all) echo ' disabled'; ?>">
                                            <a href="<?php echo ($curr_page > 1 && !$disable_all) ? 'owise.php?page=' . ($curr_page-1) . '&per_page=' . $rec_per_page : '#'; ?>">Prev</a>
                                        </li>
                                        <?php for ($i = 1; $i <= max(1, $total_pages); $i++) { ?>
                                            <li class="paginate_button <?php if ($i == $curr_page) echo 'active'; if ($disable_all) echo ' disabled'; ?>">
                                                <a href="<?php echo (!$disable_all && $i != $curr_page) ? 'owise.php?page=' . $i . '&per_page=' . $rec_per_page : '#'; ?>"><?php echo $i; ?></a>
                                            </li>
                                        <?php } ?>
                                        <li class="paginate_button next<?php if ($curr_page >= $total_pages || $disable_all) echo ' disabled'; ?>">
                                            <a href="<?php echo ($curr_page < $total_pages && !$disable_all) ? 'owise.php?page=' . ($curr_page+1) . '&per_page=' . $rec_per_page : '#'; ?>">Next</a>
                                        </li>
                                    </ul>
                                    <form method="get" class="form-inline" style="margin-bottom:0; margin-left:0; vertical-align: middle;">
                                        <select name="per_page" id="per_page" class="form-control input-sm" onchange="this.form.submit()" style="width:auto; height:34px; padding:6px 16px; margin-top:20px; margin-right:16px; font-size:14px; line-height:1.42857143; vertical-align:middle; display:inline-block;">
