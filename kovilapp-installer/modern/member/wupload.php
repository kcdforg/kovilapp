<?php
include('../popupheader.php');
$id = $_GET['id'];
$s = $id . "_wife" . ".jpg";

if ($_FILES["w_image"]["error"] > 0) {
    echo "Return Code: " . $_FILES["w_image"]["error"] . "<br />";
} else {
    //echo "images: " . $_FILES["image"]["$s"] . "<br />";
    // echo "Type: " . $_FILES["w_image"]["type"] . "<br />";
    //echo "Size: " . ($_FILES["image"]["size"] / 1024) . " Kb<br />";
    //echo "Temp file: " . $_FILES["image"]["tmp_name"] . "<br />";
    //var_dump($_FILES["image"]);
    if ($_FILES["w_image"]["type"] != 'image/jpeg') {
        echo "Image should be in jpeg format";
        die;
    } else {
        if (file_exists("../images/" . $s)) {
            echo $_FILES["w_image"]["name"] . " already exists. ";
        } else {

            move_uploaded_file($_FILES["w_image"]["tmp_name"], "images/" . $s);
            //"Stored in:" . "images/" . $s; 
            ?>
            <div class="callout callout-danger">
                <h4>Upload success!</h4>
            </div>
            <?php
        }
    }
    include('function.php');

//$image=$_FILES["image"]["$s"];


    $sql = "UPDATE `$family` SET `w_image`='$s' where `id`=$id";
//var_dump($sql);
//echo $sql;


    if (!mysql_query($sql, $con)) {
        die('Error: ' . mysql_error());
    }

}
include('../footer.php');
?>

