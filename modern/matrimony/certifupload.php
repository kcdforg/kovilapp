...
if ($_FILES["certificate"]["error"] > 0) {
    ...
    if ($_FILES["certificate"]["type"] != 'image/jpeg') {
        echo "Image should be in jpeg format";
        die;
    } else {
        if (file_exists("../attachments/" . $s)) {
            echo $_FILES["certificate"]["name"] . " already exists. ";
        } else {
            move_uploaded_file($_FILES["certificate"]["tmp_name"], "attachments/" . $s);
