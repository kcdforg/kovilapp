...
global $con;
$result_list = mysqli_query($con, "SELECT pudavai, COUNT(*) as cnt FROM $tbl_family WHERE pudavai IS NOT NULL AND TRIM(pudavai) != '' GROUP BY pudavai ORDER BY pudavai ASC");
$pudavai='';
...
if (isset($_GET['pudavai'])) {
    $pudavai = $_GET['pudavai'];
    $count_res = mysqli_query($con, "SELECT count(*) as cnt FROM $tbl_family WHERE pudavai='$pudavai'");
    $count = mysqli_fetch_array($count_res);
    $total_records = $count['cnt'];
    $total_pages = ceil($total_records / $rec_per_page);
    $result = mysqli_query($con, "SELECT * FROM $tbl_family WHERE pudavai='$pudavai' LIMIT $offset, $rec_per_page");
} else {
    $result = mysqli_query($con, "SELECT * FROM $tbl_family order by pudavai");
...
<div id="list" class="col-md-12">
    <div id="list" class="col-md-2" style="border-top: 0px;">
        <div class="row1">
            <h3 class="box-title"><b><center> Pudavai </center></b></h3>
        </div>
        <?php
        if (!$result_list) {
            ...
            while ($row = mysqli_fetch_array($result_list)) {
                ...
                <li  id="listby"class="list-group-item <?php echo $active ?>">				
                    <b><a href="pwise.php?pudavai=<?php echo $row['pudavai'] ?>"><?php echo $row['pudavai'] ?> (<?php echo isset($row['cnt']) ? $row['cnt'] : 0 ?>)</b></a>
                </li>
            ...
            <div class="row">
                ...
                <div class="box table-responsive">
                    <div class="box-header">
                        <h3 class="box-title"><b>Search Results for  <?php echo $pudavai; ?> (<?php echo $count['cnt']; ?>)</b></h3>
                    </div>
                        ...
                        <div class="dataTables_paginate paging_simple_numbers pagination-flex-align" style="margin:10px 0 10px 0; justify-content: flex-end; display: flex; align-items: center; gap: 16px;">
                            <ul class="pagination" style="margin-bottom: 0; margin-right: 0; vertical-align: middle;">
                                ...
                                <li class="paginate_button previous<?php if ($curr_page <= 1 || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page > 1 && !$disable_all) ? 'pwise.php?pudavai=' . urlencode($pudavai) . '&page=' . ($curr_page-1) . '&per_page=' . $rec_per_page : '#'; ?>">Prev</a>
                                </li>
                                <?php for ($i = 1; $i <= max(1, $total_pages); $i++) { ?>
                                    <li class="paginate_button <?php if ($i == $curr_page) echo 'active'; if ($disable_all) echo ' disabled'; ?>">
                                        <a href="<?php echo (!$disable_all && $i != $curr_page) ? 'pwise.php?pudavai=' . urlencode($pudavai) . '&page=' . $i . '&per_page=' . $rec_per_page : '#'; ?>"><?php echo $i; ?></a>
                                    </li>
                                <?php } ?>
                                <li class="paginate_button next<?php if ($curr_page >= $total_pages || $disable_all) echo ' disabled'; ?>">
                                    <a href="<?php echo ($curr_page < $total_pages && !$disable_all) ? 'pwise.php?pudavai=' . urlencode($pudavai) . '&page=' . ($curr_page+1) . '&per_page=' . $rec_per_page : '#'; ?>">Next</a>
                                </li>
                            </ul>
                            <form method="get" class="form-inline" style="margin-bottom:0; margin-left:0; vertical-align: middle;">
                                <input type="hidden" name="pudavai" value="<?php echo htmlspecialchars($pudavai); ?>">
...
?>
    <div id="list" class="col-md-10">
        <div class="row">
            <div class="box table-responsive">
                <div class="box-header">
                    <h3 class="box-title"><b>All Pudavai Members (<?php echo $total_records; ?>)</b></h3>
                </div>
                <!-- Pagination and per-page dropdown controls above the table -->
                <div class="dataTables_paginate paging_simple_numbers pagination-flex-align" style="margin:10px 0 10px 0; justify-content: flex-end; display: flex; align-items: center; gap: 16px;">
                    <ul class="pagination" style="margin-bottom: 0; margin-right: 0; vertical-align: middle;">
                        <?php $disable_all = ($total_pages <= 1); ?>
                        <li class="paginate_button previous<?php if ($curr_page <= 1 || $disable_all) echo ' disabled'; ?>">
                            <a href="<?php echo ($curr_page > 1 && !$disable_all) ? 'pwise.php?page=' . ($curr_page-1) . '&per_page=' . $rec_per_page : '#'; ?>">Prev</a>
                        </li>
                        <?php for ($i = 1; $i <= max(1, $total_pages); $i++) { ?>
                            <li class="paginate_button <?php if ($i == $curr_page) echo 'active'; if ($disable_all) echo ' disabled'; ?>">
                                <a href="<?php echo (!$disable_all && $i != $curr_page) ? 'pwise.php?page=' . $i . '&per_page=' . $rec_per_page : '#'; ?>"><?php echo $i; ?></a>
                            </li>
                        <?php } ?>
                        <li class="paginate_button next<?php if ($curr_page >= $total_pages || $disable_all) echo ' disabled'; ?>">
                            <a href="<?php echo ($curr_page < $total_pages && !$disable_all) ? 'pwise.php?page=' . ($curr_page+1) . '&per_page=' . $rec_per_page : '#'; ?>">Next</a>
                        </li>
                    </ul>
                    <form method="get" class="form-inline" style="margin-bottom:0; margin-left:0; vertical-align: middle;">
                        <select name="per_page" id="per_page" class="form-control input-sm" onchange="this.form.submit()" style="width:auto; height:34px; padding:6px 16px; margin-top:20px; margin-right:16px; font-size:14px; line-height:1.42857143; vertical-align:middle; display:inline-block;">
                ...
                <div  id="bb" class="box-body no-padding">
                    ...
                    <!-- Pagination and per-page dropdown controls below the table for All case -->
                    <div class="dataTables_paginate paging_simple_numbers pagination-flex-align" style="margin:10px 0 10px 0; justify-content: flex-end; display: flex; align-items: center; gap: 16px;">
                        <ul class="pagination" style="margin-bottom: 0; margin-right: 0; vertical-align: middle;">
                            <?php $disable_all = ($total_pages <= 1); ?>
                            <li class="paginate_button previous<?php if ($curr_page <= 1 || $disable_all) echo ' disabled'; ?>">
